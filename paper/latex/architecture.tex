...
% Record counts (drawn AFTER arrows so white bg covers them)
\node[countlabel] at (2.1, 0.0)    {231M};
\node[countlabel] at (2.1,-1.0)  {479M};
\node[countlabel] at (2.1,-2.0)  {250M};
\node[countlabel] at (2.1,-3.0)  {513K};
\node[countlabel] at (2.1,-4.0)  {69K};
\node[countlabel] at (2.1,-5.0)  {47.8M};
\node[countlabel] at (2.1,-6.0)  {146K};
\node[countlabel] at (2.1,-7.0)  {meta};
...
\node[grouplabel=duckdbyellow, anchor=south] at (10.0, 1.0)
  {DuckDB (22 schemas, 153 views)};
\node[note, anchor=north] at (10.0, 0.9)
  {view-only DB {\raise.17ex\hbox{$\scriptstyle\sim$}}270\,KB};
...
\node[note]       at (10.0,-2.90)       {\textit{+ ros, p2p, main}};
...
\draw[thickarrow=ontopurple] (tomap.east) -- ++(1.2,0) |- (ontology.west);
\node[note, anchor=north] at (12.7, -6.5) {embeddings};
