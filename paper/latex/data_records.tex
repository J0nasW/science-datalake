...
\begin{itemize}[nosep]
  \item \texttt{s2ag}: papers (133M), abstracts, citations, authors, publication
    venues from Semantic Scholar.
  \item \texttt{openalex}: works (292M), authors, institutions, sources, topics,
    concepts, publishers, funders from OpenAlex.
  \item \texttt{sciscinet}: paper metrics (159M), disruption indices, atypicality
    scores, team size indicators from SciSciNet.
  \item \texttt{pwc}: papers (141K), code links, tasks, datasets, methods from
    Papers with Code.
  \item \texttt{retwatch}: retracted papers (60K) with retraction reasons and
    dates from Retraction Watch.
  \item \texttt{ros}: patent--paper citation pairs (548K) from Reliance on Science.
  \item \texttt{p2p}: preprint-to-published DOI mappings (4M) from bioRxiv/medRxiv.
